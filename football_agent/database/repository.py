...
class DatabaseRepository:
    ...
    def upsert_odds_snapshots(self, observations: Iterable[tuple[Fixture, Sequence[OddsSnapshot]]]) -> int:
        ...
        for fixture, odds in observations:
            for snapshot in odds:
                snapshot_key = sha256_text("|".join([
                    str(fixture.id), snapshot.bookmaker, snapshot.market, snapshot.selection,
                    snapshot.timestamp_utc, f"{snapshot.odds:.6f}",
                ]))
                rows.append({
    ...
    def fetch_picks(self, limit: int = 10000) -> list[dict[str, Any]]:
        return self.client.select(
            "picks",
            columns="pick_id,identity_key,fixture_id,market,selection,status,model_version,last_seen_at",
            limit=limit,
        )
