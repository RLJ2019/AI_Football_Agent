...
from football_agent.data.odds import BookmakerProfiler, best_odds_by_selection, market_odds_matrix
...
fixtures = fixture_provider.upcoming(days_ahead=days_ahead, max_matches=max_matches)

if only_lineup_window:
    active_ids: set[str] | None = None

    if only_watchlist_lineup_monitor:
        active_ids = NotificationState(out_dir / "notification_state.json").active_fixture_ids(
            {"WATCHLIST", "VALUE_PICK"}
        )
    ...
    print(
        f"Line-up monitor actief: T-{lineup_window_start} t/m T-{lineup_window_end} minuten | "
        f"scope={scope} | wedstrijden in window: {len(fixtures)}"
    )

print(f"Wedstrijden gevonden: {len(fixtures)}")

# V25.0.9 reliability fix:
# Haal standings alleen op als er fixtures zijn gevonden.
# En haal ze alleen op voor de competities waarin daadwerkelijk fixtures zitten.
# Dit voorkomt onnodige football-data calls en voorkomt 429 rate-limit errors bij lege dagen.
standings_by_comp = {}
fixture_comp_keys = {fixture.competition_key for fixture in fixtures}

if fixtures and fixture_provider.football_data.enabled:
    print(
        "Standings ophalen voor competities met fixtures:",
        ", ".join(sorted(fixture_comp_keys)),
    )
...
for fixture in fixtures:
    ...
    if analysis.poisson:
        ...
        model_value_probs.update(
            {
                "BTTS_YES": analysis.poisson.btts.get("YES", 0.0),
                "BTTS_NO": analysis.poisson.btts.get("NO", 0.0),
            }
        )
    ...
    print(
        ...
        f"window={time_window} intl_break={post_international_break} "
        f"promoted={home_is_promoted}/{away_is_promoted} "
        f"min_edge={custom_min_edge:.1%} lineup={bool(lineups)} "
        f"odds_fresh={freshness.fresh} conf={pick.confidence:.1f} "
        f"data={pick.data_quality:.1f} unc={pick.uncertainty_score:.1f}"
    )
...
if send_value_alerts:
    for pick in picks:
        ...
        if notification.should_send and notification.action in {
            "new_value_pick",
            "upgraded_to_value_pick",
            "value_pick_changed",
        }:
            reporter.send(reporter.build_value_pick_alert(pick, notification.action), disable_notification=False)
            sent = True
            ...
            reporter.send(reporter.build_withdrawal_alert(pick), disable_notification=False)
            sent = True
