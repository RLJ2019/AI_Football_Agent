...
class ApiFootballClient:
    ...
    def _get(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            f"API-Football response endpoint={endpoint} "
            f"params={params} results={results} response_len={len(response)}"
        )
                ...
                for bet in bookmaker.get("bets", []) or []:
                    ...
                    for val in bet.get("values", []) or []:
                        ...
                        if bet_name in {"match winner", "1x2"}:
                            market = "1X2"
                            selection = (
                                "DRAW" if label in {"draw", "x"}
                                else "HOME" if label in {"home", "1"}
                                else "AWAY" if label in {"away", "2"}
                                else None
                            )
